...
ds <- open_dataset("files/tu_new.parquet")
...
final_table_indent_pdf <- final_table_wide %>%
  mutate(
    Level = ifelse(is.na(Level), "NA",
                   paste0("\\hspace{1em}", Level)),
    Male = ifelse(is.na(Male), "", Male),
    Female = ifelse(is.na(Female), "", Female)
  )

final_table_indent_pdf %>%
  arrange(Variable, Level) %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    ...
    caption = paste0(
      ...
      "Male (N = ", format(N_male, big.mark = ","), 
      ") & Female (N = ", format(N_female, big.mark=","), ")"
    ),
    col.names = c("Variable", "Level", "Male", "Female"),
    escape = FALSE
  ) %>%
...
saveRDS(final_table_indent_pdf, "final_table_indent_pdf.rds")
...
two_tables <- data.frame(
  Table2_SmokingStatusModel = paste0(
    "<div style='zoom:1.5;'>",
    kable(table_interpretation, "html"),
    "</div>"
  ),
  Table3_SmokingStatusAndDurationModel = paste0(
    "<div style='zoom:1.5;'>",
    kable(table_model7, "html"),
    "</div>"
  ),
  check.names = FALSE
)
